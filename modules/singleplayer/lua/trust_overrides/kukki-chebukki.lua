-----------------------------------
-- SINGLEPLAYER override of scripts/actions/spells/trust/kukki-chebukki.lua's
-- spellObject.onMobSpawn. Gambits are priority-ordered; this body fully
-- owns Kukki Chebukki's spawn behavior.
--
-- REBASE AUDIT: diff this against upstream's onMobSpawn body to spot new
-- gambits/mods worth porting:
--   diff <(awk '/spellObject\.onMobSpawn/,/^end$/' scripts/actions/spells/trust/kukki-chebukki.lua) \
--        <(awk '/registerSpawn.*function/,/^end\)/' modules/singleplayer/lua/trust_overrides/kukki-chebukki.lua)
-- Last reconciled: 2026-06-16 (upstream baeabac590)
-----------------------------------

local m = Module:new('trust_override_kukki_chebukki')

xi.singleplayer.trust.registerSpawn('KUKKI_CHEBUKKI', function(mob)
