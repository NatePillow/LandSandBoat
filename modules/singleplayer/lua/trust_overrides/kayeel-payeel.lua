-----------------------------------
-- SINGLEPLAYER override of scripts/actions/spells/trust/kayeel-payeel.lua's
-- spellObject.onMobSpawn. Gambits are priority-ordered; this body fully
-- owns Kayeel Payeel's spawn behavior.
--
-- REBASE AUDIT: diff this against upstream's onMobSpawn body to spot new
-- gambits/mods worth porting:
--   diff <(awk '/spellObject\.onMobSpawn/,/^end$/' scripts/actions/spells/trust/kayeel-payeel.lua) \
--        <(awk '/registerSpawn.*function/,/^end\)/' modules/singleplayer/lua/trust_overrides/kayeel-payeel.lua)
-- Last reconciled: 2026-06-16 (upstream baeabac590)
-----------------------------------

local m = Module:new('trust_override_kayeel_payeel')

xi.singleplayer.trust.registerSpawn('KAYEEL_PAYEEL', function(mob)
